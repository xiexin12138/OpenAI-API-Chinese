...
要使用的模型ID ( model ID ) ，你可以使用[模型列表](https://platform.openai.com/docs/api-reference/models/list)API来查看全部你可以使用的模型，或者查看我们的[模型概览](https://platform.openai.com/docs/models/overview)中关于它们的描述。
***
**prompt**
...
temperature采样的另一种替代方案，被称为核采样( nucleus sampling )，其中模型考虑在token的结果中排前 `top_p` 的概率质量( probability mass )。所以 0.1 意味着只考虑包含前 10% 概率质量的 tokens 。
...
`logprobs`的最大值为5，如果你需要的值大于这个，请通过[帮助中心](https://help.openai.com/)联系我们，并描述你的用例。
***
**echo**
<span style='color:#8e8ea0;font-size:13px;margin-left:10px'>boolean</span>
<span style='color:#8e8ea0;font-size:13px;margin-left:10px'>可选的</span>
